...
const char* mqtt_topic_pub = "saida/vazao"; // PUB = envia/publica dados
  ...
  // Faixas de Normalização (usados para normalizar os dados de entrada)
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  // ------------------------- INFERÊNCIA -----------------------------------
  float vazao_estimada = predict(valv_norm, temp_norm, press_in_norm, press_out_norm);
  ...
  float erro_pct = (vazao_real != 0) ? (erro_abs / vazao_real) * 100.0 : 0.0;
...
void setup() {
  ...
  Serial.println(" Aguardando dados no topico MQTT...");
}
